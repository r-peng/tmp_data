...
ham = Heisenberg(J,h,Lx,Ly,grad_by_ad=False)

burn_in = 40
sampler = ExchangeSampler(Lx,Ly,burn_in=burn_in,thresh=thresh) 
#sampler = DenseSampler(Lx,Ly,nspin,exact=True) 

tnvmc = TNVMC(ham,sampler,amp_fac,optimizer='sr',solve_full=True,solve_dense=False)
tmpdir = './' 
#tnvmc.tmpdir = tmpdir 
start = 50
stop = 70
