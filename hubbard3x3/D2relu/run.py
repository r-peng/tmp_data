...
from quimb.tensor.nn_2d import AmplitudeFNN2D
...
step = 0 
symmetry = 'z2'
model = Hubbard2D(t,u,Lx,Ly,sep=True,symmetry=symmetry)

if step==0:
    fpeps = load_ftn_from_disc(f'../D2/psi10')
else:
    fpeps = load_ftn_from_disc(f'psi{step}_0')
for tid,tsr in fpeps.tensor_map.items():
    #print(tid,tsr.phase)
    tsr.phase = {}
af1 = FermionAmplitudeFactory2D(fpeps,model,symmetry=symmetry)

lr = [None] * 1
nv = af1.nsite * 2
nh = 50
lr[0] = Dense(nv,nh,'relu')
if step==0:
    eps = 1e-2
    w,b = relu_init_normal(nv,nh,-1,1)
    lr[0].params[0] = w * eps
    lr[0].params[1] = b * eps
af2 = AmplitudeFNN2D(Lx,Ly,lr)
af2.const = 0
af2.log = True 
af2.phase = False
af2.fermion = True
