...
if step==0:
    fpeps = load_ftn_from_disc(f'../fpeps/psi150')
    config = np.load(f'../fpeps/config149.npy')
else:
...
for tid,tsr in fpeps.tensor_map.items():
    #print(tid,tsr.phase)
    tsr.phase = {}
af0 = FermionAmplitudeFactory2D(fpeps,spinless=spinless,symmetry=symmetry,max_bond=chi)
af0.model = model 
af0.spin = None 
